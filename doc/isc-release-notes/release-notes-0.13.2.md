ISC Core version 0.13.2 is now available from:

  <https://download.isc.org/isc-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/isc-project/isc/issues>
...
updates will be released anymore. Without security updates, using a isc
...
ECDSA signatures inside ISC transactions now use validation using
...
Previous versions of ISC Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of ISC Core was to raise the default minimum
relay fee.

ISC Core 0.13.2 will have a strict maximum size on the mempool. The
...
ISC Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running iscd without having to do any manual configuration.
...
ISC Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. ISC Core will relay transactions with
...
entirely in ISC Core 0.13, and it is at this time undecided whether the
...
ISC Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
ISC Core automatically creates a hidden service to listen on, without
manual configuration. ISC Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if ISC Core is listening, and
...
ISCd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals ISC
Core to use floating fees. By default, ISC Core will use floating
fees.
...
(default: `0.0002` ISC/kB).

At all times, ISC Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ISC.
Furthermore, ISC Core will never create transactions paying less than
...
`isc.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /iscrpc>
        ...
        # optional bypass iscd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< iscrpc:<password>
    </Location>
...
Restarting iscd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in isc.conf
...
For example:
    ...
    $ src/isc-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The ISC Core code base has
...
- `isc-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `isc-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the ISC scripting system that in combination with
...
ISC transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of ISC Core.
...
The two phases are distinct in the ISC-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming ISC wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each ISC block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of iscs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of iscs being spent is exactly
...
- **Increased security for multisig:** ISC addresses (both P2PKH addresses
  ...
  believed to be provided by ISC's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of ISC's current ledger (called the UTXO set),
  ...
  note that ISC Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ISC users to individually opt-in to almost any change in the ISC
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and ISC Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the ISC
client to ISC Core 0.13.2 to signal for segwit support. When a
...
Since ISC Core 0.10.0, nodes have defaulted to only relaying and
...
- `isc-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `isc-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
